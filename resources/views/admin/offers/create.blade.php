        ...
        <div class="col-md-10">
            <div class="card shadow-sm border-0">
                <div class="card-header bg-white py-3">
                    <h5 class="mb-0 text-primary">Create New Offer</h5>
                </div>
                    ...
                    <form action="{{ route('admin.offers.store') }}" method="POST">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" class="form-control @error('offer_title') is-invalid @enderror" id="offer_title" name="offer_title" value="{{ old('offer_title') }}" required autofocus placeholder="e.g., Summer Super Sale">
                                @error('offer_title')
                        ...
                        <div class="row mb-4">
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-select @error('offer_type') is-invalid @enderror" id="offer_type" name="offer_type" required onchange="toggleTargetFields()">
                                    ...
                                    <option value="festival" {{ old('offer_type') == 'festival' ? 'selected' : '' }}>Festival Offer</option>
                                    <option value="product" {{ old('offer_type') == 'product' ? 'selected' : '' }}>Product-wise Offer</option>
                                    <option value="category" {{ old('offer_type') == 'category' ? 'selected' : '' }}>Category-wise Offer</option>
                                    <option value="first_order" {{ old('offer_type') == 'first_order' ? 'selected' : '' }}>First Order Offer</option>
                                    <option value="flash_sale" {{ old('offer_type') == 'flash_sale' ? 'selected' : '' }}>Flash Sale</option>
                                    <option value="seasonal" {{ old('offer_type') == 'seasonal' ? 'selected' : '' }}>Seasonal Sale</option>
                                </select>
                                @error('offer_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="row mb-4">
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="number" step="0.01" min="0" max="100" class="form-control @error('discount_value') is-invalid @enderror" id="discount_value" name="discount_value" value="{{ old('discount_value') }}" required>
...
<script>
    ...
    // Run on load to set initial state in case of validation errors
